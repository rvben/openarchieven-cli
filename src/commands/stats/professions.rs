...
pub fn run(
    ...
    if ctx.fields.is_some() {
        return Err(Error::new(
            ...
            "--fields is not supported for `stats professions`; use `-o json | jq`",
        ));
    }
...
pub fn parse_rest(rest: &[String]) -> Result<Args> {
    ...
    const KNOWN: &[&str] = &["--place", "--year-start", "--year-end"];
    ...
    while let Some(tok) = iter.next() {
        ...
        let (key, val) = if let Some(eq) = s.find('=') {
            ...
            if KNOWN.contains(&key) {
                ...
                return Err(Error::new(
                    ErrorKind::Validation,
                    format!("unknown flag {key} for `stats professions`"),
                ));
            } else {
                return Err(Error::new(
                    ErrorKind::Validation,
                    format!("unexpected positional {s} for `stats professions`"),
                ));
            }
            ...
            return Err(Error::new(
                ErrorKind::Validation,
                format!("unknown flag {s} for `stats professions`"),
            ));
        } else {
            return Err(Error::new(
                ErrorKind::Validation,
...
mod tests {
    ...
    #[test]
    fn parse_empty_place_is_validation_error() {
